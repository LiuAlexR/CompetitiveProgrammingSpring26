...
/// Basketball Together - Greedy
fn p_a(scanner: &mut Scanner) {
    let ND = scanner.read_vec_usize();
    let mut P = scanner.read_vec_usize();
    P.sort();
    let n = ND[0];
    let d = ND[1];
    let mut count = 0;
    ...
    let mut cur_num_players = 0;
    while top > bottom {
        let num_teams = d / P[top];
        if d % P[top] == 0 {
            bottom = bottom + num_teams - 1;
        } else {
            bottom = bottom + num_teams;
        }
        top = top - 1;
        if top >= bottom {
            count = count + 1;
