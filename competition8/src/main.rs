...
fn p_c(scanner: &mut Scanner) {
    ...
    for _ in 0..t {
        ...
        let r1 = scanner.read_string().as_bytes().to_vec();
        let r2 = scanner.read_string().as_bytes().to_vec();

        let mut dp = vec![0; n + 1];

        for i in 1..=n {
            let cost_v = if r1[i-1] == r2[i-1] { 0 } else { 1 };
            dp[i] = dp[i-1] + cost_v;

            if i >= 2 {
                let cost_h1 = if r1[i-2] == r1[i-1] { 0 } else { 1 };
                let cost_h2 = if r2[i-2] == r2[i-1] { 0 } else { 1 };
                dp[i] = dp[i].min(dp[i-2] + cost_h1 + cost_h2);
            }
        }
